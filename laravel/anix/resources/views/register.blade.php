...
<main>
    ...
    <form action="{{route('register')}}" method="POST">
        @csrf
        <input type="text" name="name" id="username" placeholder="Ваш никнейм">
        <input type="email" name="email" id="email" placeholder="Логін або email">
        <input type="password" name="password" id="password" placeholder="Пароль">
        <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Підтвердження Пароля">
        <div class="org">
            <a href="{{route('login')}}">Маєш аккаунт?</a>
